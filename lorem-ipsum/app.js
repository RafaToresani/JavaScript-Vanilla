...
const textGenerated = document.querySelector('.lorem-text');
const template = document.querySelector(".templateText").content;
const fragment = document.createDocumentFragment();

form.addEventListener('submit', (e)=>{
  e.preventDefault();
  textGenerated.textContent='';
  ...
  for(let i=0; i<value; i++){
    ...
    clone.querySelector(".p").textContent=text[index];
    fragment.appendChild(clone);
  }
  textGenerated.appendChild(fragment);
})

function getRandomArbitrary() {
  return Math.floor(Math.random() * (text.length - 0) + 0);;
}